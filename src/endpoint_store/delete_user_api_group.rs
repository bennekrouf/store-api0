...
pub async fn delete_user_api_group(
    ...
    if !group_still_used {
        ...
        for endpoint_id in &endpoint_ids {
            let endpoint_still_used: bool = tx
                .query_row(
            ...
            if !endpoint_still_used {
                ...
                // Delete endpoint
                tx.execute("DELETE FROM endpoints WHERE id = ?", [endpoint_id])
                    .to_store_error()?;
            }
        }

        // Delete the group itself (if it's not a default group)
        tx.execute("DELETE FROM api_groups WHERE id = ?", [group_id])
            .to_store_error()?;
    }

use crate::endpoint_store::{EndpointStore, StoreError, ApiGroup, ApiGroupWithEndpoints, Endpoint, Parameter};
use duckdb::ToSql;
...
fn fetch_default_groups_with_endpoints(
    ...
    tracing::debug!("Fetching default groups and endpoints");
...
fn fetch_endpoints(
    ...
    let endpoint_rows_iter = stmt.query_map(params, |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, String>(1)?,
            row.get::<_, String>(2)?,
